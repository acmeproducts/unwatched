...
export class Metrics implements Brain {
  ...
  async digest(ctx: DigestContext): Promise<DigestText> { const b = this.bucket(); b.t2++; const out = await this.timed(b, () => this.inner.digest(ctx)); this.meter(b, this.models.stakes); return out; }
