...
export function clockOf(town: Town) {
  return { t: town.t, day: town.day, minute: town.minuteOfDay, hour: town.hour, label: town.clock(), weather: town.weather, season: town.season, population: town.agents.size, flourShortage: town.flourShortage, weekday: town.weekdayName, occasion: town.occasion, voices: !!(process.env.GEMINI_API_KEY ?? process.env.GOOGLE_API_KEY), ...(realClock.place ? { place: realClock.place, temperatureC: realClock.temperatureC ?? null, sunrise: realClock.sunrise ?? null, sunset: realClock.sunset ?? null } : {}) };
}
