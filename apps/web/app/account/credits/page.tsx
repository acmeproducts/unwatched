      ...
      <div className="grid gap-5 grow grid-cols-1 lg:grid-cols-[minmax(0,1fr)_400px] xl:grid-cols-[260px_minmax(0,1fr)_420px]">
        ...
        <div className="flex flex-col gap-4 min-h-0">
          ...
          <Card className="px-7"><div className="flex justify-between items-baseline"><h2 className="text-[22px] font-semibold">Plan</h2><span className="text-[13px] text-drift">{w.plans[w.plan].name}</span></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">{(Object.keys(w.plans) as Plan[]).map((k) => { const pl = w.plans[k]; const cur = k === w.plan; return <div key={k} className={`rounded-[20px] p-5 flex flex-col gap-1 ${cur ? "bg-glass" : "bg-sand"}`} style={cur ? { boxShadow: "inset 0 0 0 2px #1F5F5B" } : undefined}><div className="font-bold">{pl.name}</div><div className="display text-2xl font-bold">${pl.price}{pl.price ? <span className="text-sm font-semibold text-drift"> / mo</span> : null}</div><div className="text-[13px] text-ink2">{pl.blurb}</div><div className="text-[12px] text-drift">{pl.tier1} routine, {pl.tier2} with stakes{pl.reflect ? ", nightly reflection" : ""}</div>{cur ? <span className="text-[12px] font-bold text-teal mt-1.5">Current plan{k !== "visitor" && !w.testMode ? <> · <button onClick={manage} disabled={busy} className="underline">Manage on Stripe</button></> : null}</span> : <button onClick={() => (k === "visitor" && !w.testMode && w.plan !== "visitor" ? manage() : choose(k))} disabled={busy} className="text-[12px] font-bold text-teal mt-1.5 text-left">{k === "visitor" ? "Downgrade" : "Choose"}</button>}</div>; })}</div>
          </Card>
