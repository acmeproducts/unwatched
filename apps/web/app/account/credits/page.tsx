    ...
    <Page>
      <div className="grid gap-5 grow grid-cols-1 lg:grid-cols-[minmax(0,1fr)_400px] xl:grid-cols-[260px_minmax(0,1fr)_420px]">
        ...
        <div className="flex flex-col gap-4 min-h-0">
          <Card className="px-7"><div className="flex flex-col sm:flex-row justify-between sm:items-baseline gap-1"><h1 className="text-[26px] font-semibold">Credits</h1><span className="text-[13px] text-drift">{w.testMode ? "Test mode: no card is charged" : "Before tax, in US dollars"}</span></div>
          ...
          <Card className="px-7"><div className="flex justify-between items-baseline"><h2 className="text-[22px] font-semibold">Plan</h2><span className="text-[13px] text-drift">{w.plans[w.plan].name}</span></div>
            {w.plan === "none" && <p className="text-sm text-ink2 max-w-[70ch]"><b>{first} has no plan</b> and lives on habit: work, meals, sleep, set phrases. Nobody thinks for free on the island; the Visitor is the least that keeps a mind going.</p>}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">{(["visitor", "resident", "patron"] as Plan[]).map((k) => { const pl = w.plans[k]; const cur = k === w.plan; return <div key={k} className={`rounded-[20px] p-5 flex flex-col gap-2 ${cur ? "bg-glass" : "bg-sand"}`} style={cur ? { boxShadow: "inset 0 0 0 2px var(--color-teal)" } : undefined}><div className="flex justify-between items-baseline"><div className="font-bold text-[17px]">{pl.name}</div><div className="display text-2xl font-bold">${pl.price}<span className="text-sm font-semibold text-drift"> / mo</span></div></div><div className="text-[13px] text-ink2">{pl.blurb}</div><ul className="text-[13px] text-ink2 flex flex-col gap-1 pl-4 m-0 list-disc">{pl.gets.map((g) => <li key={g}>{g}</li>)}</ul>{cur ? <span className="text-[12px] font-bold text-teal mt-auto pt-1">Current plan{!w.testMode ? <> · <button onClick={manage} disabled={busy} className="underline">Manage on Stripe</button></> : null}</span> : <button onClick={() => choose(k)} disabled={busy} className="text-[12px] font-bold text-teal mt-auto pt-1 text-left">{w.plan === "none" ? "Choose" : pl.price > w.plans[w.plan].price ? "Move up" : "Move down"}</button>}</div>; })}</div>
            {w.plan !== "none" && <p className="text-[12px] text-drift">Prices are per citizen, per month, before tax. Ending the plan is done on Stripe's page; the citizen then lives on habit until a plan is bought again.</p>}
          </Card>
