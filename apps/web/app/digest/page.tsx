    ...
    <Page>
      <div className="grid gap-5 grow grid-cols-1 lg:grid-cols-[minmax(0,1fr)_380px]">
        <div className="bg-shell rounded-[28px] px-5 py-6 sm:px-10 sm:py-9 flex flex-col gap-5 rise">
          ...
          <div className="pl-0 sm:pl-7 rise" style={{ "--i": 2 } as React.CSSProperties}><Strip items={d.items.map((e) => ({ t: `${dayOf(e.t) === dayOf(d.now) ? "Today" : `Day ${dayOf(e.t)}`} ${hhmm(e.t)}`, changed: e.importance >= 0.45, text: <>{e.kind === "conversation" ? talk(e.text) : e.text}{e.payload?.because ? <span className="block text-[13px] text-drift italic">because {String(e.payload.because).replace(/[.]$/, "")}</span> : null}</> }))} /></div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {d.letters.length === 0 && <Card><Label>Letters</Label><div className="display text-xl font-semibold">Nothing yet.</div><p className="text-sm text-ink2">{first} writes when something is at stake, usually within the first three days. You can write first.</p><LinkButton href="/letters" kind="secondary" size={36}>Write to {first}</LinkButton></Card>}
          <Card><Label>People</Label>{agent.people.length ? agent.people.slice(0, 5).map((p) => <Tide key={p.id} name={p.name} trust={p.trust} word={p.tide} />) : <p className="text-sm text-drift">Nobody yet. Trust grows with every conversation, and shrinks without them.</p>}</Card>
          {(agent.watch?.length || agent.selves?.length) ? <Card><Label>Who {first} is becoming</Label>{agent.watch?.length ? <p className="text-sm"><span className="text-drift">Keeping an eye on: </span>{agent.watch.join(", ")}</p> : null}{agent.selves?.length ? <details className="text-sm"><summary className="cursor-pointer text-ink2">{agent.selves.length === 1 ? "Rewrote themself once" : `Rewrote themself ${agent.selves.length} times`}; now wants {String(agent.persona.want ?? "")}</summary>{agent.selves.slice().reverse().map((sv) => <div key={sv.day} className="mt-2 pl-3 border-l border-line"><div className="text-xs text-drift">until day {sv.day}</div><div>wanted {sv.want} · feared {sv.fear}</div></div>)}</details> : null}</Card> : null}
