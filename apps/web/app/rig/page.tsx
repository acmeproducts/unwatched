...
export default function Rig() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const more: [string, (c: Citizen) => void][] = [["angry", (c) => c.mood({ anger: 1 })], ["surprised", (c) => c.mood({ surprise: 1 })], ["tired", (c) => c.mood({ tired: 1 })], ["broke", (c) => c.wear({ broke: true })], ["a roof of their own", (c) => c.wear({ roof: true })], ["apron", (c) => c.trade("cook at the bakery")]];
      ...
      newPoses.forEach(([name, pose], i) => { const c = new Citizen({ ...base, hair: "Bob", carrying: "Basket", top: "Teal", bottom: "Sand", beard: "None", glasses: false, hairColor: "Dark", pattern: "Plain", shape: "Straight" }); c.scale.set(S); c.position.set(770 + i * 110, 1280); c.setPose(pose); if (pose === "argue") c.mood({ anger: 0.8 }); if (pose === "greet") c.mood({ joy: 0.7 }); stage.addChild(c); rigs.push({ c }); label(name, 770 + i * 110, 1288); });
