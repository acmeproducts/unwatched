...
function furnish(g: Graphics, kind: string, sprite: string, W: number, H: number, floorY: number): { seats: [number, number][]; beds: [number, number][]; benches: [number, number][] } {
  ...
  switch (kind) {
    ...
    case "civic": table(W / 2 - 70, 140); g.roundRect(W / 2 - 16, 18, 32, 40, 3).fill(CLOTH).stroke({ width: 1.2, color: INK }); break;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const p of people.slice(0, 6)) {
        ...
        const c = new Citizen(lookFor(p.name, p.appearance as Partial<Look> | null)); c.scale.set(1.45); c.setPose(pose);
