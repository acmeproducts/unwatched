...
export function Painting({ scene, edition }: { scene: PaperScene; edition: number }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      app = new Application();
      await app.init({ width: W, height: H, background: 0xefede4, antialias: true, resolution: Math.min(2, window.devicePixelRatio || 1), autoDensity: true });
      ...
      const top = night ? 0x16262b : scene.weather === "storm" ? 0x4a5a63 : scene.weather === "rain" || scene.weather === "fog" ? 0x9aa9ad : dusk ? 0xf0b98a : 0xbfe0ea;
      const bottom = night ? 0x2a3f45 : scene.weather === "storm" ? 0x7a8790 : scene.weather === "rain" || scene.weather === "fog" ? 0xc7d0d2 : dusk ? 0xf7dcc0 : 0xe6f2f3;
      ...
      stage.addChild(sky);
      if (night) { for (let i = 0; i < 40; i++) sky.circle((i * 173) % W, (i * 97) % (H * 0.45), i % 5 === 0 ? 1.6 : 1).fill({ color: 0xfff6d5, alpha: 0.7 }); sky.circle(W - 110, 70, 22).fill(0xfff2c2); }
      ...
      const sea = new Graphics(); sea.rect(0, H * 0.58, W, H * 0.08).fill(night ? 0x1f3a42 : scene.weather === "storm" ? 0x4c6a74 : 0x5fa8b3); stage.addChild(sea);
      const ground = new Graphics(); ground.rect(0, H * 0.64, W, H * 0.36).fill(night ? 0x3a4a44 : scene.weather === "rain" || scene.weather === "storm" ? 0x9c9c86 : 0xcfc4a2); ground.ellipse(W / 2, H * 0.78, 300, 44).fill({ color: 0xffffff, alpha: night ? 0.04 : 0.18 }); stage.addChild(ground);
      // the place
      ...
      const frame = new Graphics(); frame.rect(0, 0, W, H).stroke({ width: 10, color: 0xefede4 }); frame.rect(5, 5, W - 10, H - 10).stroke({ width: 1.5, color: 0x1e2a2b, alpha: 0.5 }); stage.addChild(frame);
      const cap = new Text({ text: `${scene.placeName}, ${String(scene.hour).padStart(2, "0")}:00 · ${scene.weather} · plate ${edition}`, style: { fontFamily: "Nunito Sans, sans-serif", fontSize: 12, fontWeight: "700", fill: 0x1e2a2b } });
      const capBg = new Graphics(); capBg.roundRect(14, H - 34, cap.width + 20, 24, 12).fill({ color: 0xefede4, alpha: 0.92 }); stage.addChild(capBg); cap.position.set(24, H - 29); stage.addChild(cap);
