...
export function Loading({ what = "The ferry office is slow this morning." }: { what?: string }) {
...
export function SignedOut({ what }: { what: string }) {
  return <Card className="max-w-[560px]"><Label>Ferry office</Label><h1 className="text-[28px] font-bold">Nobody is signed in.</h1><p className="text-ink2">{what}</p><div className="flex gap-2"><LinkButton href="/gate">Sign in</LinkButton><LinkButton href="/town" kind="secondary">Watch the town</LinkButton></div></Card>;
}
export function NoAgent({ what }: { what: string }) {
  return <Card className="max-w-[560px]"><Label>Ferry office</Label><h1 className="text-[28px] font-bold">You have nobody on the island yet.</h1><p className="text-ink2">{what}</p><LinkButton href="/board">Board the ferry</LinkButton></Card>;
}
export function Problem({ text, retry }: { text: string; retry?: () => void }) {
  return <Card tone="sand" className="max-w-[560px]"><Label>That did not work</Label><p className="text-ink2">{text} Nothing was lost.</p>{retry && <Button kind="secondary" size={36} onClick={retry}>Try again</Button>}</Card>;
}
