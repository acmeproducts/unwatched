...
const D: Record<string, () => Drawn> = {
  ...
  mill: () => {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      const at = (t: number, side: number): Pt => [hub[0] + ux * t + nx * side, hub[1] + uy * t + ny * side];
      g.moveTo(hub[0], hub[1]).lineTo(at(len, 0)[0], at(len, 0)[1]).stroke({ width: 3.5, color: KELP });
      poly(g, [at(14, 0), at(len - 2, 0), at(len - 2, sw), at(14, sw)], CREAM);
      for (let t = 20; t < len - 2; t += 8) g.moveTo(at(t, 0)[0], at(t, 0)[1]).lineTo(at(t, sw)[0], at(t, sw)[1]).stroke({ width: 1, color: KELP, alpha: 0.7 });
      g.moveTo(at(14, sw / 2)[0], at(14, sw / 2)[1]).lineTo(at(len - 2, sw / 2)[0], at(len - 2, sw / 2)[1]).stroke({ width: 1, color: KELP, alpha: 0.7 });
    }
    g.circle(hub[0], hub[1], 4).fill(CORAL).stroke(S);
    return { c: g, w: w + d + 50 };
  },
  ...
  // props
  "tree-large": () => { const g = new Graphics(); g.moveTo(0, 0).lineTo(0, -22).stroke({ width: 6, color: WOOD_DARK }); treeBall(g, -14, -34, 17, TEAL); treeBall(g, 14, -36, 16, TEAL_DARK); treeBall(g, 0, -50, 19, TEAL); treeBall(g, -4, -30, 12, SAGE_DARK); return { c: g, w: 72 }; },
  "tree-small": () => { const g = new Graphics(); g.moveTo(0, 0).lineTo(0, -14).stroke({ width: 4, color: WOOD_DARK }); treeBall(g, -8, -22, 11, SAGE); treeBall(g, 8, -24, 10, SAGE_DARK); treeBall(g, 0, -32, 12, SAGE); return { c: g, w: 44 }; },
