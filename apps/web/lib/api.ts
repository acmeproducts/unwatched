...
export type PublicAgent = { id: string; name: string; age: number; origin: string; summary: string; location: string; place: string; asleep: boolean; job: string | null; home: string | null; arrivedDay: number; funded: boolean; ownerId: string | null; appearance: Record<string, unknown> | null };
export type Person = { id: string; name: string; trust: number; affection: number; opinion: string; lastSeen: number; tide: string };
export type OwnerAgent = PublicAgent & { persona: Record<string, unknown>; needs: { hunger: number; rest: number; social: number }; coins: number; inventory: string[]; nightsPaid: number; budget: { tier1Left: number; tier2Left: number; tier1Max: number; tier2Max: number }; intentions: string[]; people: Person[]; memories: { t: number; text: string; importance: number; kind: string }[]; letters: { id: number; text: string; t: number; read: boolean }[] };
export type Digest = { headline: string; items: TownEvent[]; people: { name: string; trust: number; opinion: string }[]; since: number; now: number; agent: OwnerAgent | PublicAgent; letters: { t: number; text: string }[] };
