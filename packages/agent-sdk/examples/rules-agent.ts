 ...
 *   UW_TOKEN=ft_agent_... pnpm --filter @unwatched/agent-sdk example
 */
import { connect } from "../src/index.ts";
import type { Action, Perception } from "@unwatched/protocol";

const token = process.env.UW_TOKEN; if (!token) { console.error("Set UW_TOKEN to the token from Account, Who thinks."); process.exit(1); }
